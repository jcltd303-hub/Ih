 ...
 */
export const getEconomyStats = onCall(async (request) => {
  if (!request.auth?.uid) {
  ...
  let completed = 0;
  let pending = 0;
  const byDay: Record<string, { depositsUsd: number; gc: number; sc: number; count: number }> = {};
  ...
  if (depositsSnap) {
    depositsSnap.forEach((doc) => {
      ...
      if (!byDay[day]) byDay[day] = { depositsUsd: 0, gc: 0, sc: 0, count: 0 };
      if (status === 'completed') {

...
export const processPlayerShot = onCall(async (request) => {
  ...
  const result = await db.runTransaction(async (transaction) => {
    ...
    // clientHitConfirmed flag (see AUDIT_REPORT.md for the bigger
    // trust-boundary issue). What this closes is a narrower gap: the same
    // targetId being submitted more than once within a session to collect
    // the kill payout repeatedly. All reads must happen before any writes
    // in this transaction, so this read happens up front regardless of the
    // eventual roll outcome.
    ...
    return {
      ...
      // Expose the nonce range consumed so a client/auditor can later
      // recompute these exact rolls once the session seed is revealed.

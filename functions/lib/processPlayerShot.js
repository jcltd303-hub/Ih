...
function rtpTelemetryShard(userId) {
    let hash = 0;
...
function evaluateServerHit(betAmount, fishType, skinBonus, payoutTable, rng) {
 ...
 */
exports.processPlayerShot = (0, https_1.onCall)(async (request) => {
    const data = request.data || {};
    const { sessionId, currencyType, betAmount, targetId, clientHitConfirmed, clientKillConfirmed, fishType: rawFishType, skinBonus: rawSkinBonus, timestamp, requestId } = data;
    const userId = request.auth?.uid;
    if (!userId) {
        throw new https_1.HttpsError('unauthenticated', 'User must be authenticated to fire shots.');
    }
    if (currencyType !== 'SC' && currencyType !== 'GC') {
        throw new https_1.HttpsError('invalid-argument', 'Invalid currency.');
    }
    (0, limits_1.assertBetAmount)(betAmount, currencyType);
    await (0, limits_1.assertRateLimit)(userId, 'shot', limits_1.MAX_SHOTS_PER_MINUTE);
    if (!sessionId || typeof sessionId !== 'string' || !sessionId.startsWith('sess_')) {
        throw new https_1.HttpsError('invalid-argument', 'A committed fairness session is required.');
    }
    if (!requestId || typeof requestId !== 'string' || typeof timestamp !== 'number') {
        throw new https_1.HttpsError('invalid-argument', 'Missing request fields.');
    }
    if (Math.abs(Date.now() - timestamp) > 60000) {
        throw new https_1.HttpsError('invalid-argument', 'Request timestamp expired.');
    }
    // Fish tier is server-authoritative.
    const fishType = rawFishType === 'boss'
        ? 'boss'
        : rawFishType === 'medium'
            ? 'medium'
            : 'small';
    ...
    const result = await db.runTransaction(async (transaction) => {
        ...
        if (requestDoc.exists) {
            throw new https_1.HttpsError('already-exists', 'Duplicate request.');
        }
        const sessionSnap = await transaction.get(sessionRef);
        if (!sessionSnap.exists || sessionSnap.data()?.status !== 'active') {
            throw new https_1.HttpsError('failed-precondition', 'No active fairness session. Call startGameSession first.');
        }
        ...
        if (session.payoutTable && typeof session.payoutTable === 'object') {
            try {
                payoutTable = (0, payoutTable_1.validatePayoutTable)(session.payoutTable);
            }
            catch {
                throw new https_1.HttpsError('failed-precondition', 'Invalid payout table attached to session.');
            }
        }
        const startNonce = Number(session.nonce) || 0;
        const walletDoc = await transaction.get(userWalletRef);
        if (!walletDoc.exists) {
            throw new https_1.HttpsError('not-found', 'User wallet not found. Call ensureUserWallet first.');
        }
        ...
        if (currentBalance < betAmount) {
            throw new https_1.HttpsError('failed-precondition', 'Insufficient funds for bet.');
        }
        ...
        if (currencyType === 'SC') {
            ...
            if (existingNetLoss >= limits_1.MAX_DAILY_SC_LOSS) {
                throw new https_1.HttpsError('failed-precondition', `Daily SC loss cap (${limits_1.MAX_DAILY_SC_LOSS}) reached.`);
            }
        }
        const isCollision = Boolean(clientHitConfirmed) && typeof targetId === 'string' && targetId !== 'pending_collision';
        ...
        if (currencyType === 'SC' && netLoss > 0 && existingNetLoss + netLoss > limits_1.MAX_DAILY_SC_LOSS) {
            throw new https_1.HttpsError('failed-precondition', `Daily SC loss cap (${limits_1.MAX_DAILY_SC_LOSS}) reached.`);
        }
        ...
        if (payoutAmount > 0) {
            ...
            const winCap = currencyType === 'SC' ? limits_1.MAX_CLAIMED_WIN_SC_PER_MINUTE : limits_1.MAX_CLAIMED_WIN_GC_PER_MINUTE;
            if (claimedInWindow > winCap) {
                throw new https_1.HttpsError('resource-exhausted', `Claimed win burst limit reached for ${currencyType}. Please slow down and try again shortly.`);
            }
        }
        ...
        return {
            ...
            sweepstakesCoins: balanceKey === 'sweepstakesCoins' ? finalBalance : Number(walletData.sweepstakesCoins) || 0,
            hit: hitResult,
            kill: killResult,
            killed,
...
//# sourceMappingURL=processPlayerShot.js.map
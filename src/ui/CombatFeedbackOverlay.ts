...
export class CombatFeedbackOverlay {
  ...
  constructor(parent: HTMLElement) {
    ...
    this.root.innerHTML = `
      <div class="ff-fighter-strip">
        <span class="ff-fighter-label">FIGHT</span>
        <span class="ff-fighter-divider"></span>
        <span id="ff-fighter-status">READY</span>
        <span class="ff-fighter-divider"></span>
        <span id="ff-fighter-kills">K.O. 00</span>
      </div>
      <div id="ff-fighter-score" class="ff-fighter-score">00</div>
      <div id="ff-fighter-hit" class="ff-fighter-hit" aria-hidden="true"></div>`;
  ...
  private bind(): void {
    ...
    this.unsubs.push(bus.on<THEME_CHANGED_EVENT>('THEME_CHANGED', (e) => { this.theme = e?.theme === 'dark' ? 'dark' : 'light'; }));
    ...
    this.unsubs.push(bus.on<TurretMultiplierEvent>('TURRET_UPDATE', (e) => {
      if (e.state === 'COOLDOWN') this.setStatus('TURRET RECHARGE', 'cooldown');
      else if (e.active && e.remainingMs <= 550) this.setStatus('TURRET END', 'turret');
    }));
  ...
  private onHit(e: FishHitEvent, boss = false): void {
    ...
    const crit = !!e.isCrit;
    const grade = crit ? (damage >= 25 ? 'CRITICAL!' : 'CRITICAL') : damage >= 20 ? 'POWER HIT' : boss ? 'BOSS HIT' : 'HIT';
    this.hit.textContent = `${grade}  ${damage}`;
    this.hit.dataset.kind = crit ? 'critical' : boss ? 'boss' : 'normal';
  ...
  private setStatus(text: string, kind: string): void {
    ...
    if (kind && !['boss', 'ko', 'danger'].includes(kind)) {
      this.hideTimer = window.setTimeout(() => {
        this.status.textContent = 'FIGHT';
...
type THEME_CHANGED_EVENT = { theme?: 'light' | 'dark' };

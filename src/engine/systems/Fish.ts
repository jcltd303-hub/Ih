...
export class Fish implements Boid {
  ...
  constructor(id: string, type: 'small' | 'medium' | 'boss', startX: number, startY: number,
    ...
    if (isBoss) {
      ...
      if (type === 'small') this.animRig.container.scale.set(0.72);
      else if (type === 'medium' && theme === 'light') this.animRig.tint(0xa5f3fc);
  ...
  public updateSteering(neighbors: Fish[], screenWidth: number, screenHeight: number, threatX?: number, threatY?: number, dtScale = 1): void {
    ...
    if (newFacing !== this.facing) { this.facing = newFacing; this.animRig?.playState(this.facing === 'left' ? 'swim_left' : 'swim_right'); }
    this.container.x = this.x; this.container.y = this.y;
    this.bounds.x = this.x - this.width / 2; this.bounds.y = this.y - this.height / 2;
  }

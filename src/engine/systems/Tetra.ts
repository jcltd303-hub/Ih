...
/** Authored Tetra cutout assembled from the nine source parts as one fish-shaped puppet. */
export class Tetra extends Container {
  ...
  public setTheme(theme: Theme): void {
    ...
    for (const sprite of [this.sTorsoMain, this.sHeadJaw, this.sEyeballLarge, this.sEyeballSmall,
  ...
  public update(dtScale = 1, vx = 1): void {
    ...
    const tailWave = Math.sin(t * 5.2 - 0.8);
    const finWave = Math.sin(t * 5.2 - 1.5);

    // Keep the body recognizable and let the movement travel from head to tail.
